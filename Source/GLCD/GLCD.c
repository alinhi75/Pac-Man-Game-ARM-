...
void LCD_DrawRectangle(int x, int y, int width, int height, int color) {
	int i , j;
    for ( i = x; i < x + width; i++) {
        for ( j = y; j < y + height; j++) {
            LCD_SetPoint(i, j, color);  // Assumes a function to set a pixel
        }
    }
}

void LCD_DrawCircle(int x, int y, int radius, int color) {
		int i , j;
    for ( i = -radius; i <= radius; i++) {
        for ( j = -radius; j <= radius; j++) {
            if (i * i + j * j <= radius * radius) {
                LCD_SetPoint(x + i, y + j, color);  // Assumes a function to set a pixel
            }
        }
    }
}

void LCD_DrawFilledCircle(int x0, int y0, int radius, unsigned short color) {
	int r;
    for ( r = radius; r >= 0; r--) {
        LCD_DrawCircle(x0, y0, r, color); // Draw concentric circles to fill
    }
}
// Helper function to check if a point is inside the triangle
    int isInside(int px, int py) {
        ...
        int area1 = (x1 - x0) * (py - y0) - (y1 - y0) * (px - x0);
        int area2 = (x2 - x1) * (py - y1) - (y2 - y1) * (px - x1);
        int area3 = (x0 - x2) * (py - y2) - (y0 - y2) * (px - x2);

        // Check if all areas are positive or all negative
        return (area1 >= 0 && area2 >= 0 && area3 >= 0) || (area1 <= 0 && area2 <= 0 && area3 <= 0);
    }
void LCD_DrawTriangle(int x0, int y0, int x1, int y1, int x2, int y2, int color) {
    int minX = x0, maxX = x0;
    int minY = y0, maxY = y0;
	int x,y;
    ...
    // Iterate through bounding box and fill pixels inside the triangle
    for ( x = minX; x <= maxX; x++) {
        for ( y = minY; y <= maxY; y++) {
            if (isInside(x, y)) {
                LCD_SetPoint(x, y, color); // Draw the pixel if inside the triangle
            }
        }
    }
}
...
void LCD_FillRect(int x, int y, int width, int height, uint16_t color) {
    ...
    for ( row = 0; row < height; row++) {
        LCD_DrawLine(x, y + row, x + width - 1, y + row, color);
    }
}
void LCD_DrawFilledTriangle(int x0, int y0, int x1, int y1, int x2, int y2, int color) {
    int minX = x0, maxX = x0;
    int minY = y0, maxY = y0;
    int x, y;
    ...
    // Helper function to check if a point (x, y) is inside the triangle
    int area2 = abs((x1 - x0) * (y2 - y0) - (x2 - x0) * (y1 - y0)); // Total triangle area x2

    // Iterate through bounding box and fill pixels inside the triangle
    for (x = minX; x <= maxX; x++) {
        for (y = minY; y <= maxY; y++) {
            int a1 = abs((x - x0) * (y1 - y0) - (x1 - x0) * (y - y0)); // Sub-triangle 1 area x2
            int a2 = abs((x - x1) * (y2 - y1) - (x2 - x1) * (y - y1)); // Sub-triangle 2 area x2
            int a3 = abs((x - x2) * (y0 - y2) - (x0 - x2) * (y - y2)); // Sub-triangle 3 area x2

            // If the sum of sub-triangle areas equals the total triangle area, point is inside
            if (a1 + a2 + a3 == area2) {
                LCD_SetPoint(x, y, color); // Draw the pixel if inside the triangle
            }
        }
...
void LCD_DrawFilledHeart(int x, int y, int size, int color) {
    int halfSize = size / 2;

    // Draw two top triangles for the heart (representing circles)
    LCD_DrawFilledTriangle(
        x - size, y,
        x - halfSize, y - size,
        x, y,
        color
    ...
    // Draw bottom triangle for the heart
    LCD_DrawFilledTriangle(
        x - size, y,
        x + size, y,
        x, y + size,
